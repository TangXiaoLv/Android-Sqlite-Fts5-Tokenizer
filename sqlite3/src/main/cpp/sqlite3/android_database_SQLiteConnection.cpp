...
*/
    ...
    static jlong nativeOpen(JNIEnv *env, jclass clazz, jstring pathStr, jint openFlags,
        ...
        /* init fts5 tokenizer*/
        if (init_icucompat() != SQLITE_OK) {
            throw_sqlite3_exception(env, db, "Failed to load ICU library.");
            return 0;
        }

        // Register fts5 tokenizer utility functions.
        if (sqlite3_register_mm_utils(db) != SQLITE_OK) {
            throw_sqlite3_exception(env, db, "Failed to register mm utils.");
            return 0;
        }
...
// Called when a custom function is destroyed.
    ...
    static int
    executeOneRowQuery(JNIEnv *env, SQLiteConnection *connection, sqlite3_stmt *statement) {
        int err = sqlite3_step(statement);
        if (err != SQLITE_ROW) {
            throw_sqlite3_exception(env, connection->db);
        }
